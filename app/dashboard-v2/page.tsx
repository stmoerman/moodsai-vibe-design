...
export default function DashboardV2() {
  ...
  // Find next upcoming appointment for countdown
  ...
  // Determine which appointments are past
  ...
  // Determine which appointment is "next" (first upcoming)
  ...
  const renderAgendaContent = (inExpandedDialog: boolean) => (
    <>
      ...
      {/* Time-to-next countdown */}
      {nextAppointment && dateIndex === 1 && viewMode === 'dag' && (
        <div className={s.countdownLine}>
  ...
  return (
    <div className={s.root}>
      ...
      {/* Welcome Hero + AI Insight Banner */}
      <div className={s.welcomeHero}>
        ...
        <div className={s.aiBriefing}>
          <span className={s.aiBriefingSparkle}>&#10022;</span>
          <span className={s.aiBriefingText}>
            6 sessies vandaag &middot; M. de Vries liet verbetering zien (GAD-7 &#8595;4) &middot; 1 intake gepland &middot; Declarabiliteit op schema
          </span>
        </div>
      ...
      {activeTab === 'directe-tijd' && (
        <div className={s.tabContent}>
          <div className={s.tabContentHeader}>
            ...
            <span className={s.tabContentMeta}>Week 13 &middot; 24–28 maart 2026</span>
          </div>
      ...
      {activeTab === 'agenda' && <main className={s.mainGrid}>
        ...
        {/* Left Column — Key Stats + Messages */}
        <div className={s.leftCol}>
          {/* Stat: Cliënten vandaag */}
          <div className={s.statBlock} style={{ animationDelay: '0s' }}>
            <div className={s.statLabel}>Cli&euml;nten vandaag</div>
            <div className={s.statValue}>6</div>
            <div className={s.statDots}>
              {[true, true, true, true, false, false].map((filled, i) => (
                <span key={i} className={filled ? s.dotFilled : s.dotHollow} />
              ))}
            </div>
            <div className={s.statSub}>4 bevestigd &middot; 2 in afwachting</div>
          </div>

          {/* Stat: Declarabiliteit */}
          <div className={s.statBlock} style={{ animationDelay: '0.03s' }}>
            <div className={s.statLabel}>Declarabiliteit</div>
            <div className={s.statValue}>81%</div>
            <div className={s.progressBarWrap}>
              <div className={s.progressBarTrack}>
                <div className={s.progressBarFill} style={{ width: '81%' }} />
                <div className={s.progressBarTarget} style={{ left: '80%' }} />
              </div>
            </div>
            <div className={s.statSub}>Target: 80%</div>
          </div>
          ...
          {/* Stat: Ongelezen */}
          <div className={s.statBlock} style={{ animationDelay: '0.06s' }}>
            <div className={s.statLabel}>Ongelezen</div>
            <div className={s.statValueWarm}>
              3
              <span className={s.warmDotIndicator} />
            </div>
          ...
          <section className={s.messagesSection}>
            ...
            <div className={s.messagesScroll}>
              {messages.map((msg, i) => (
                <div key={i} className={`${s.messageRow} ${isRecentMessage(msg.time) ? s.messageRowRecent : ''}`}>
        ...
        <div className={s.rightCol}>
            ...
            <div className={s.askMoodyBody}>
              {askMoodyInsights.map((insight, i) => (
                <div key={i} className={s.insightCard}>
                  <span className={s.insightText}>{insight.text}</span>
                </div>
              ))}
            </div>
            <input
            ...
            {recentNotes.map((note, i) => (
              <div key={i} className={s.noteRow}>
                <span className={s.noteText}>
                  {note.text}
                  {note.aiGenerated && <span className={s.aiTag}>AI gegenereerd</span>}
                </span>
          ...
          <a href="#" target="_blank" rel="noopener noreferrer" className={s.roomLinkCard}>
            <span className={s.roomLinkContent}>
              Mijn kamer
              <span className={s.roomLinkStatus}>
                <span className={s.roomLinkGreenDot} />
                Beschikbaar
              </span>
            </span>
